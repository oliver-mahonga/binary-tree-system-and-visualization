...
function getRoot() {
    ...
    startTraversalHighlighting(root[0]);  // Start traversal after tree is created
    return root;
}
...
function startTraversalHighlighting(root) {
    const inOrder = inOrderTraversal(root);
    const preOrder = preOrderTraversal(root);
    const postOrder = postOrderTraversal(root);

    document.getElementById('inOrderTraversal').innerHTML = inOrder.join(' ');
    document.getElementById('preOrderTraversal').innerHTML = preOrder.join(' ');
    document.getElementById('postOrderTraversal').innerHTML = postOrder.join(' ');
}
